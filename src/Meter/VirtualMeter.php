        ...
        <head>
            ...
            because we are providing our own feedback */
                ...
                #settings-link:active {
                    ...
                    transform: scale(0.9);
                    /* Slight shrink gives a "button press" feel */
                    opacity: 1;
                }
            ...
            <script>
                ...
                function initDisplay(flatSNS = null) {
                    ...
                    configMetrics.forEach((m, idx) => {
                        ...
                        let unit = m.unit || '';
                        if (m.large) {
                            if (label.length > maxLargeLabelLen) {
                                maxLargeLabelLen = label.length;
                                maxLargeLabel = label;
                            }
                            if ((value + unit).length > maxLargeValueLen) {
                                maxLargeValueLen = (value + unit).length;
                                maxLargeValue = value;
                                maxLargeUnit = unit;
                            }
                        } else {
                            if (label.length > maxNormalLabelLen) {
                                maxNormalLabelLen = label.length;
                                maxNormalLabel = label;
                            }
                            if ((value + unit).length > maxNormalValueLen) {
                                maxNormalValueLen = (value + unit).length;
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    // --- SETTINGS LINK ---
                    // Standard link behavior (no preventDefault here either)
                    ...
                    interactiveSymbols.forEach(el => {
                        ...
                        if (title) {
                            ...
                            /**
                             * We use 'click' for desktop and most touch interactions.
                             * Removing e.preventDefault() is the key to re-enabling 
                             * browser-level gestures like pinch-to-zoom.
                             */
                            el.addEventListener('click', function(e) {
                                // If it's a touch device (iPad), show the title as an alert
                                if (window.matchMedia("(pointer: coarse)").matches) {
                                    alert(title.textContent);
                                    // e.preventDefault() REMOVED: Allowing events to bubble up 
                                    // ensures the browser can still handle zooming.
                                }
                            }, {
                                passive: true
                            }); // 'passive: true' explicitly tells the browser not to wait
                        }
                    });
                });
            </script>
